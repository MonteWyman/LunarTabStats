...
public class OverlayCommand extends Command {
    ...
    public void handle(String[] args) {
        ...
        CompletableFuture.runAsync(() -> {
            ...
            Collection<NetworkPlayerInfo> playerInfos = List.copyOf(Minecraft.getMinecraft().getNetHandler().getPlayerInfoMap());
            ...
            for (NetworkPlayerInfo playerInfo : playerInfos) {
                ...
                PackagedProfile packagedProfile = new PackagedProfile(gameProfile);
                if (HyCache.isCached(gameProfile.getName())) {
                    packagedProfile.setProfile(HyCache.getByName(gameProfile.getName()));
                    profiles.add(packagedProfile);
                    continue;
                }
                EntityPlayer entityPlayer = Minecraft.getMinecraft().theWorld.getPlayerEntityByName(gameProfile.getName());
                ...
                if (entityPlayer != null && !entityPlayer.onGround && HyUtils.check(entityPlayer.getName())) {
                ...
                try {
                    ...
                    HyProfile hyProfile = new HyProfile(gameProfile.getId().toString(), gameProfile);
                    //System.out.println("Time Used: " + (System.currentTimeMillis() - start));
                    //System.out.println("玩家 " + gameProfile.getName() + " 的统计数据已获取");
                    packagedProfile.setProfile(hyProfile);
                    profiles.add(packagedProfile);
                    HyCache.cacheProfile(hyProfile, gameProfile.getName());
                    ...
                    System.out.println("获取玩家: " + gameProfile.getName() + " 的数据时遇到错误");
            ...
            for (PackagedProfile packagedProfile : sorted_profiles) {
                if (!packagedProfile.isNicked()) {
                    ...
                    if (profile == null) {
                        ...
                        Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText("§a玩家: §e" + packagedProfile.getGameProfile().getName() + " §e警告: §7[§c数据获取失败§7]"));
                        continue;
                    }
                    double fkdr = (double) profile.getBw_final_kill_long() / (double) profile.getBw_final_death_long();
                    Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText("§a玩家: §e" + profile.getGameProfile().getName() + " §aBedwars等级: §e" + profile.getLevel() + " §aFKDR: §e" + String.format("%.2f", fkdr) + " §a最终击杀: §e" + profile.getBw_final_killed() + " §a胜场: §e" + profile.getBw_win_games()));
                } else {
                    Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText("§a玩家: §e" + packagedProfile.getGameProfile().getName() + " §e警告: §7[§cNICKED§7]"));
                }
            }
        });
    }
